...
export class UserMapper {
  ...
  static toPersistence(domainEntity: User): UserEntity {
    ...
    let status: GenericStatusEntity | undefined = undefined;

    if (domainEntity.status) {
      status = new GenericStatusEntity();
      status.id = Number(domainEntity.status.id);
    }

    let userType: UserTypeEntity | undefined = undefined;

    if (domainEntity.type) {
      userType = new UserTypeEntity();
      userType.id = Number(domainEntity.type.id);
    }
    ...
    persistenceEntity.type = userType;
