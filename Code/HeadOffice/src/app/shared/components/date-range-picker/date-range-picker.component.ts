...
export class DateRangePickerComponent {
  ...
  onDocumentClick(event: MouseEvent) {
    ...
    if (this.showPicker && this.pickerContainer?.nativeElement) {
      const clickedInside = this.pickerContainer.nativeElement.contains(event.target);
      if (!clickedInside) {
  ...
  togglePicker(event?: Event) {
    ...
    const wasOpen = this.showPicker;
    this.showPicker = !this.showPicker;

    if (this.showPicker && !wasOpen) {
