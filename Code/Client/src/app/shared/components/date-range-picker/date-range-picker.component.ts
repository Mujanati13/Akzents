...
export class DateRangePickerComponent {
  ...
  // Add host listener to close picker when clicking outside
  ...
  onDocumentClick(event: MouseEvent) {
    ...
    // Skip if we don't have the container reference
    ...
    // Check if the click is inside the picker container (includes trigger button and picker dropdown)
    const clickedInside = this.pickerContainer.nativeElement.contains(target);
    
    // Close only if clicked outside the entire container
    if (!clickedInside) {
      this.showPicker = false;
      // Use detectChanges to immediately update the view since we're using OnPush
      this.cdr.detectChanges();
    }
  }

  togglePicker(event?: Event) {
    // Stop event propagation and prevent default to prevent document click handler from firing
    if (event) {
      event.stopPropagation();
      event.preventDefault();
      // Mark the event so we can identify it in the document handler if needed
      (event as any)._pickerToggle = true;
    }
  ...
  selectPreset(range: { start: Date | null; end: Date | null }) {
    ...
    if (start && end) {
      ...
      // Don't close - let user close manually or click outside
    }
  ...
  handleDateClick(date: Date): void {
    ...
    // Case 2: Only start date is selected, now selecting end date
      ...
      // Emit the complete range immediately but don't close the popup
