...
export class NotificationsComponent implements OnInit {
  ...
  onSeenToggle(notification: UserNotification): void {
    ...
    if (notification.conversation?.id) {
      ...
      this.notificationsService.toggleConversationSeen(notification.conversation.id)
        .pipe(
          catchError((error) => {
        ...
        .subscribe({
          next: () => {
            ...
            this.notifications = this.notifications.map(n => 
              n.conversation?.id === notification.conversation?.id 
                ? { ...n, seen: newSeenState }
                : n
            );
            this.updateUnseenCount();
      ...
      this.notificationsService.toggleSeen(notification.id)
        .pipe(
          catchError((error) => {
  ...
  onNotificationClick(notification: UserNotification): void {
    ...
    if (!notification.seen) {
      if (notification.conversation?.id) {
        ...
        this.notificationsService.markConversationAsSeen(notification.conversation.id)
          .pipe(
            catchError((error) => {
        ...
        this.notificationsService.markSeen(notification.id)
          .pipe(
            catchError((error) => {
